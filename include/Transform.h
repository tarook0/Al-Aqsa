 ...
 *
 * NOTE: The "global space" is actually the parent's space if the GameObject (or
 * any other class that have a parent) has a parent present.
 */
class Transform {
private:
  glm::vec3 position;
...
public:
   ...
   *
   * The created transformation is constructed by scaling, rotating and
   * translating in this order.
   */
  Transform(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f),
            glm::qua<double> rotation = glm::qua<double>(1.0f, 0.0f, 0.0f,
                                                         0.0f),
  ...
  /** \brief Translates (moves) the object from ::position to ::position + \a
   * translation (in local space) ,Overwrites ::position to the new location.
   *
   * \param translation Specifies the vector (in local space) translating the
   * object from its current ::position.
   */
  void translate(glm::vec3 translation);
  /** \brief Translates (moves) the object from ::position to ::position + \a
   * translation (in global space) ,Overwrites ::position to the new location.
   *
   * \param translation Specifies the vector (in global space) translating the
   * object from its current ::position.
   */
